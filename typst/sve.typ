...
//  Diseño: editorial neutro. Color reservado a niveles de riesgo.
// ════════════════════════════════════════════════════════════
...
#set page(
  ...
  header: {
    ...
    let brand = if D.org.tradeName != none { D.org.tradeName } else { "Programa de Vigilancia Epidemiológica" }
    grid(
      columns: (1fr, auto),
      align: (left, right),
      upper(brand),
...
// ─── Encabezados ────────────────────────────────────────────
// Capítulos en romano; secciones como "4.1" para seguir la convención del sector.
...
#let note-block(body, accent: ink) = block(
  ...
  text(font: sans, size: 8pt, fill: ink2, body)
)
